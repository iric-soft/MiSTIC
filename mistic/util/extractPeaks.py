...
class Extract_peak():
    ...
    def print_peak(self, min_gene, min_diff_corr, outF=None):
        ...
        diff_corr = (self.max_corr-self.min_corr)
        if min_diff_corr != 0 and min_diff_corr > diff_corr:
            return genes_in_peaks
        ...
        peaks = "" + self.id + "\t" + str(len(self.genes)) + "\t" \
                + str(self.min_corr) + "\t" + str(self.max_corr) + "\t" \
                + genes_in_peaks
...
def merge_peak(node1, node2, corr, min_gene, max_gene, min_diff_corr,
               max_diff_corr, peaks, outF=None):
    peaks_str = []
    ...
    if node1.peak_id == "0":
        ...
        if peak.valid_peak and not stay_valid:
            peak_str = peak.print_peak(min_gene, min_diff_corr, outF)
            if len(peak_str) != 0:
                peaks_str.append(peak_str)
            ...
            if both_valid:
                ...
                if max_diff_corr != 0:
                    stay_valid = stay_valid \
                                and (peak1.delta_peaks(peak2) <= max_diff_corr)

                if not stay_valid:
                    if peak1.valid_peak:
                        peak_str = peak1.print_peak(min_gene, min_diff_corr,
                                                    outF)
                        if len(peak_str) != 0:
                            peaks_str.append(peak_str)
                        peak1.invalid_peak()
                    if peak2.valid_peak:
                        peak_str = peak2.print_peak(min_gene, min_diff_corr,
                                                    outF)
                        if len(peak_str) != 0:
                            peaks_str.append(peak_str)
                        peak2.invalid_peak()
...
def read_nodes(reader):
    ...
    while len(line) != 0 and line[0] == "n":
        ...
        id_node = attribute[2].split("=")[1]
        nodes[attribute[1]] = Extract_node(attribute[1], id_node, line)
...
def extract_peaks(reader, last_line, nodes, min_gene, max_gene, min_diff_corr,
                  max_diff_corr, outF=None):
    peaks = {}
    peaks_str = []
    line = last_line
    cpt_id = 1
    ...
    while len(line) != 0:
        ...
        corr = float(attribute[3].split("=")[1])

        if (node1.peak_id != "0" or node2.peak_id != "0"):
            peak_tmp = merge_peak(node1, node2, corr, min_gene, max_gene,
                                  min_diff_corr, max_diff_corr, peaks, outF)
            peaks_str = peaks_str + peak_tmp
        else:
            create_peak(node1, node2, corr, peaks, str(cpt_id), max_diff_corr)
            cpt_id += 1
...
def exec_extract_peaks(inF, min_gene, max_gene, min_diff_corr, max_diff_corr,
    ...
    peaks_str = extract_peaks(reader_in, last_line, nodes, min_gene, max_gene,
                              min_diff_corr, max_diff_corr, writter_out)
